 ...
 * @author frans
 */
public class importRating extends javax.swing.JFrame {

    /**
     * Creates new form importRatings
     */
    String filePath = "";
    int total = 0;
    int lastRow = 0;
    ...
    private void btn_importFileMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btn_importFileMouseClicked
        ...
        if (chooser.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                label_countData.setText(datatypeSheet.getPhysicalNumberOfRows()-1 + " DATA LOADED");
    ...
    private void btn_startImportMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_btn_startImportMouseClicked
        ...
        try {
            FileInputStream fileStream = new FileInputStream(filePath);
            M_Ratings mRating = new M_Ratings();
            Workbook workBook = new XSSFWorkbook(fileStream);
            Sheet datatypeSheet = workBook.getSheetAt(0);
                ...
                if (nextRow == null) {
                    ...
                    while (cellIterator.hasNext()) {
                        ...
                        if (columnIndex == 0) {
                            ...
                            mRating.setUser_id(Integer.parseInt(userId));
                        } else if (columnIndex == 1) {
                            Cell isi_kolom_skrg = datatypeSheet.getRow(nextCell.getRowIndex()).getCell(columnIndex);
                            String bookIsbn = formatter.formatCellValue(isi_kolom_skrg);
                            mRating.setBook_isbn(bookIsbn);
                        } else if (columnIndex == 2) {
                            Cell isi_kolom_skrg = datatypeSheet.getRow(nextCell.getRowIndex()).getCell(columnIndex);
                            String bookRating = formatter.formatCellValue(isi_kolom_skrg);
                            mRating.setBook_rating(Double.parseDouble(bookRating));
                        }
                    }
                }
                total++;
                System.out.println("PROCESSING " + total + " OF " + lastRow + " DATAS");
                new C_Ratings().tambah_Rating(mRating);
